...
export async function getStaticPaths() {
  let paths = allworkspaces.workspaces.map(workspace => ({
    params: {
      workspace: [encodeURIComponent(workspace.friendly_name)]
    }
  }))
  paths.push({
...
export default function New({ workspace }) {
  ...
  useEffect(() => {
    if(workspace === null) {
      ...
      const workspaceDetails = allworkspaces.workspaces.find(el => el.friendly_name === decodeURIComponent(workspace[0]))
